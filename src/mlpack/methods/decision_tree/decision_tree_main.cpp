...
static void mlpackMain()
{
  ...
  RequireParamValue<int>("minimum_leaf_size", [](int x) { return x > 0; }, true, "leaf size must be positive");

  RequireParamValue<int>("maximum_depth", [](int x) { return x >= 0; }, true, "maximum depth must not be negative");

  RequireParamValue<double>("minimum_gain_split", [](double x)
                         { return (x > 0.0 && x < 1.0); }, true, "gain split must be a fraction in range [0,1]");
  ...
  if (IO::HasParam("training"))
  {
    ...
    if (IO::HasParam("test"))
    {
        ...
        CheckInputShape("decision_tree", trainingSet.n_rows, trainingSet.n_cols, 1, labels.n_elem,  testPoints.n_rows, testPoints.n_cols);
    }
    else
      CheckInputShape("decision_tree", trainingSet.n_rows, trainingSet.n_cols, 1, labels.n_elem, -1, -1); 
