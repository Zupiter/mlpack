...
static void mlpackMain()
{
  ...
  if (IO::HasParam("training"))
  {
    ...
    if (IO::HasParam("test"))
    {
        ...
        CheckInputShape("logistic_regreesion", data.n_rows, data.n_cols, 1, labels.n_elem,  testPoints.n_rows, testPoints.n_cols);
    }
    else
      CheckInputShape("logistic_regression", data.n_rows, data.n_cols, 1, labels.n_elem, -1, -1);
